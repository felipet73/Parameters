...
namespace S10Cuantificacion.CollectorExts
{
    public class ExtensibleStorageEntityContentStream : IElementStream
    {
        ...
        public void Stream(Type type)
        {
            ...
            data.Add(new Data.ExtensibleStorageSeparator());
        ...
        private void AddFieldValue(Field field, object value)
        {
            try
            {
                if (field.ContainerType != ContainerType.Simple)
                    data.Add(new Data.Enumerable(field.FieldName, value as IEnumerable));
                else if (field.ValueType == typeof(double))
                    data.Add(new Data.Double(field.FieldName, (double)value));
                else if (field.ValueType == typeof(string))
                    data.Add(new Data.String(field.FieldName, value as string));
                else if (field.ValueType == typeof(XYZ))
                    data.Add(new Data.Xyz(field.FieldName, value as XYZ));
                else if (field.ValueType == typeof(UV))
                    data.Add(new Data.Uv(field.FieldName, value as UV));
                else if (field.ValueType == typeof(int))
                    data.Add(new Data.Int(field.FieldName, (int)value));
                else if (field.ValueType == typeof(ElementId))
                    data.Add(new Data.ElementId(field.FieldName, value as ElementId, document));
                else if (field.ValueType == typeof(Guid))
                {
                    var guidValue = (Guid)value;

                    data.Add(new Data.String(field.FieldName, guidValue.ToString()));
                }
                else
                    data.Add(new Data.Object(field.FieldName, value));
            }
            catch (Exception ex)
            {
                data.Add(new Data.Exception(field.FieldName, ex));
